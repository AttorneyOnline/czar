...
To set up Czar for development, follow these instructions.
...
If you don't want to use Git, you can download the latest zip of Czar [here](https://github.com/AttorneyOnline/czar/archive/refs/heads/master.zip). Extract it and put it wherever you want.
...
* You don't need to copy characters into the `characters` folder *unless* you specifically chose to disable iniswapping in an area (in `areas.yaml`). In this case, all Czar needs to know is the `char.ini` of each character. It doesn't need sprites.
...
You can also use docker to run Czar. First you need to install [Docker](https://get.docker.com/) and [Docker Compose](https://docs.docker.com/compose/install/).

Once you have everything configured, do `docker-compose up`. It will build the image and start Czar up for you. If you accidentally restart the server, the container will automatically start back up. If you're not understanding why it's starting, try starting it up manually:

## Pro Tips

* To keep the server running even if your login shell is closed, use a multiplexer, such as screen or tmux.
* For more info about available command, see [Commands](https://github.com/AttorneyOnline/czar/blob/master/docs/commands.md). You may also use the /help command on the server.
